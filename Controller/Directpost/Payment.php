 ...
 */
namespace WorldnetTPS\Payment\Controller\Directpost;
...
abstract class Payment extends \Magento\Framework\App\Action\Action
{
     ...
     * Get session model
     *
     * @return \WorldnetTPS\Payment\Model\Directpost\Session
     */
    protected function _getDirectPostSession()
    {
        return $this->_objectManager->get('WorldnetTPS\Payment\Model\Directpost\Session');
    }
    ...
    protected function _responseAction($area = 'frontend')
    {
        ...
        $data['x_invoice_num'] = $this->getRequest()->getParam('x_invoice_num');
        $data['x_response_code'] = $this->getRequest()->getParam('x_response_code');
        $data['x_response_reason_code'] = $this->getRequest()->getParam('x_response_reason_code');
        $data['x_trans_id'] = $this->getRequest()->getParam('x_trans_id');
        /* @var $paymentMethod \WorldnetTPS\Payment\Model\DirectPost */
        $paymentMethod = $this->_objectManager->create('WorldnetTPS\Payment\Model\Directpost');

        $result = [];
        if (!empty($data['x_invoice_num']) && $data['x_response_code'] && !empty($data['x_trans_id'])) {
            $result['x_invoice_num'] = $data['x_invoice_num'];
            $params['order_success'] = $helper->getSuccessOrderUrl($result);
        } else
            if (!empty($data['x_invoice_num']) && $data['x_response_code']==0) {
                $result['x_invoice_num'] = $data['x_invoice_num'];
                $params['order_success'] = $helper->getFailureOrderUrl($result);
            }
        ...
        try {
            ...
            $result['success'] = $data['x_response_code'];
