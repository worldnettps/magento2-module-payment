...
directPost.prototype = {
    initialize : function(methodCode, iframeId, controller, orderSaveUrl, cgiUrl, nativeAction) {
        var prepare = function (event, method) {
            if (method === 'worldnettps_directpost') {
                this.preparePayment();
            } else {
                jQuery('#edit_form')
                    .off('submitOrder.worldnettps');
            }
        };
        ...
        this.inputs = ['cc_type', 'cc_name', 'cc_number', 'expiration', 'expiration_yr', 'cc_cid'];
        this.headers = [];
        this.isValid = true;
    ...
    preparePayment : function() {
        ...
        jQuery('#edit_form')
            .off('submitOrder')
            .on('submitOrder.worldnettps', this.submitAdminOrder.bind(this));
        if ($(this.iframeId)) {
    ...
    preparePaymentRequest : function(data) {
        ...
        data.x_card_name = $(this.code + '_cc_name').value;
